...
<style>
  ...
  .active {
    ...
    color: #fe657b;
    text-shadow: -4px -4px 10px rgba(254, 101, 123, 0.25),
...
<div class="con float-right absolute right-0 bottom-0 mr-20">
  ...
  <div class="nav h-12 w-full">
    <p
      class={current === 'Snip' ? 'active ml-6 mt-6 mb-0 absolute' : 'nav-op ml-6 mt-6 mb-0 absolute'}
      on:click={() => (current = 'Snip')}>
      Snippets
    </p>
    <p
      class={current === 'Likes' ? 'active ml-24 mt-6 mb-0 absolute' : 'nav-op ml-24 mt-6 mb-0 absolute'}
      on:click={() => (current = 'Likes')}>
      Likes
    </p>
  </div>
